 ...
 *
 * Adapted for ACMStream.DLL by Valery V. Anisimovsky (son_of_the_north@mail.ru)
 *
...
CACMUnpacker::CACMUnpacker (FileReadFunction readFunc, int fileHandle)
:	fileBuffPtr (NULL),
	decompBuff (NULL),
	someBuff (NULL)
{
	...
	try {
		...
		if (!someBuff) throw;
		Amplitude_Buffer = new short[0x10000];
		if (!Amplitude_Buffer) throw;
...
unsigned char CACMUnpacker::readNextPortion() {
	read_file (hFile, (void*)fileBuffPtr, (unsigned long)bufferSize, (unsigned long*)&availBytes);
	if (!availBytes) {
