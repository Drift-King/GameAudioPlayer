...
#define ID_WAVE     0x46464952          // WAVE header ('RIFF')
#define ID_WAVE2    0x45564157          // WAVE header ('WAVE')
#define ID_SMK      0x324B4D53          // SMK header ('SMK2')
#define ID_PCX      0x0801050A
...
//  TMPQFile   * prev;                  // 34 : Previous file handle pointer
    ...
    // Non-Storm items
    CHAR         fileName[MAX_PATH+1];  // File name (Entry is not used by Storm.dll)
    DWORD        fileIndex;
};
...
BOOL  WINAPI SFileReadFile(HANDLE hFile, VOID * buffer, DWORD toRead, DWORD * read = NULL, int = 0);
