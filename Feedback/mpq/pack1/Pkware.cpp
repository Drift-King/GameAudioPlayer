/*****************************************************************************/
/* Pkware.cpp                                Copyright Project Software 1999 */
/*                                                                           */
...
//  DWORD         DwordESP18;           // [ESP+18] -
    ...
    // 15006A8C
    // EBP = i;
    for(DWORD i = 0; i < 0x100; itemPtr++, i++)
    {
...
//          (15006870)   ECX              ESP+14
{
    ...
    do
    {
        ...
        while((LONG)(temp2 = temp1->prev) > NULL)  // EBX
        {
...
//           (15006CD0)   ECX                 EDX               ESP+            ESP+
{
    ...
    THTreeItem * itemAt7;               // [ESP+20] - HTree item found at 7th bit
...
static void Call1502C8E0(LONG count, BYTE * buffer1, BYTE * buffer2, BYTE * buffer3)
//          (1502C8E0)   ECX         EDX             [ESP+14]        [ESP+18]
{
	for(count = count-1; count >= 0; count--)   // EBX - count
	{
        DWORD IntEAX = buffer2[count];
        DWORD IntEBP = 1 << buffer1[count];

		do
		{
            buffer3[IntEAX] = (char)count;
			IntEAX         += IntEBP;
        }
        while(IntEAX < 0x100);
	}
...
static void Call1502C920(TPKWAREStruct * ps)
//          (1502C920)   ECX
{
    DWORD index   = 0xFE;                            // EDX
    WORD * source = (WORD *)Table15034860 + index;   // ESI
    DWORD EAX, EBP;
    ...
    do
    {
        BYTE * target = &ps->buff2FB4[index];       // EBP
        BYTE   CL     = *target;
        if(CL <= 8)
        {
            // 1502C941:
        ...
        else
        {
            ...
            {
                // EAX &= 0xFFFF;
                ps->buff2C34[EAX & 0xFFFF] = (BYTE)0xFF;
    ...
    while(source >= (WORD *)Table15034860);
}

static DWORD DoDecompression4(TPKWAREStruct * ps)
//           (1502C530)       ECX
{
	...
	while(1)
	{
        ...
        if((oneByte = GetOneByte(ps)) >= 0x305)
            break;
...
static DWORD PrepareDecompression4(FILLINPUTBUFF FillInputBuffer, FILLOUTPUTBUFF FillOutputBuffer, TPKWAREStruct * ps, void * userData)
//           (1502C3C0)            ECX                            EDX                              [ESP+10]            [ESP+14]
{
    ...
    if(4 <= ps->rBits && ps->rBits <= 6) 
    {
        ...
        Call1502C8E0(0x10, ps->nSkip2, Table15034750, ps->position2);
        ...
        Call1502C8E0(0x40, ps->nSkip1, Table150346D0, ps->position1);

        return (DoDecompression4(ps) == 0x305) ? 0 : 0x0F;
    }
    // 1502C517:
    return 1;
}
...
//----------------------------------------------------------------------------
// void Decompression4(BYTE * outBuffer, DWORD * pOutLength, BYTE * inpBuffer, DWORD inpLength)
...
//   (15007680)     ECX               EDX               EDX               [ESP+3A98]
{
    ...
    PrepareDecompression4(CopySourceData, CopyTargetData, &ps, &info);
}
...
//   (150081F0)        [ESP+1C],         [ESP+20],           [ESP+24],         [ESP+28]
{
    ...
    cflags = *inpBuffer++;              // EDX = Compression flags (The first byte of the input data)
    inpLength--;                        // EBP = inpLength - 1
