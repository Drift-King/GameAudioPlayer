...
static DWORD decryptHashIndex(char * fileName, DWORD * stormBuffer)
{
    ...
    while(*key != 0)
    {
        ...
        seed1 = stormBuffer[0x000 + ch] ^ (seed1 + seed2);
...
static DWORD decryptName1(char * fileName, DWORD * stormBuffer)
{
    ...
    while(*key != 0)
    {
        ...
        seed1 = stormBuffer[0x100 + ch] ^ (seed1 + seed2);
        seed2 = ch + seed1 + seed2 + (seed2 << 5) + 3;
    }
    return seed1;
}
...
static DWORD decryptName2(char * fileName, DWORD * stormBuffer)
{
    ...
    while(*key != 0)
    {
        ...
        seed1 = stormBuffer[0x200 + ch] ^ (seed1 + seed2);
        seed2 = ch + seed1 + seed2 + (seed2 << 5) + 3;
    }
    return seed1;
}
...
// If fileName is given by number (less or equal to number of files),
...
BOOL WINAPI SFileOpenFileEx(HANDLE hArchive, char * fileName, int something, HANDLE * hFile)
{
    ...
    if(hArchive == NULL || ((DWORD)fileName > ha->header->nFiles && *fileName == 0) || hFile == NULL)
    {
    ...
    hf->fileIndex = blockIndex;
    if((DWORD)fileName > 0x00400000)
        strcpy(hf->fileName, fileName);

    // Decrypt file seed. Cannot be used if the file is given by index
    if((DWORD)fileName > ha->header->nFiles)
    {
        char * plainName = fileName;
    ...
    // If nameless access, we have to determine file seed
    if((DWORD)fileName < ha->header->nFiles && (hf->block->flags & MPQ_FILE_ENCRYPTED))
    {
        char buffer[32];

        if(SFileGetFileName(hf, buffer) == FALSE)
        {
            delete hf;
            return FALSE;
        }
