...
static void AddRNode(RFile ** first, RFile ** last, const CHAR * fileName, LONG index = -1)
{
    RFile * node;                       // Resource file node

    node = (RFile*)GlobalAlloc(GPTR, sizeof(RFile));
    
    // If file given by index, use rfDataString as TMPQDataString structure
    if(index != -1)
    {
        TMPQDataString * data = (TMPQDataString *)(node->rfDataString);

        data->dummy = MAGIC_DUMMY;
...
static void SaveDialog(HWND hdlg, TFileListInfo * info)
{
    ...
    for(index = 0; index < info->fileCount; index++)
    {
        ...
        if(ListBox_GetSel(hList, index) == TRUE)
        {
            ...
            // If nameless access, add file by index
            if(!strnicmp(fileName, "File", 4))
                AddRNode(&list, &last, fileName, atol(&fileName[4]));
            else
